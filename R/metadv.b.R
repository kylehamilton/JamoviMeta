...
metaDVClass <- if (requireNamespace('jmvcore')) R6::R6Class(
    "metaDVClass",
    inherit = metaDVBase,
    private = list(
      .run = function() {
        ...
        if (ready == TRUE) {
          ...
          if (self$options$includemods == TRUE) {
            data <- data.frame(yi = self$data[[self$options$effectSize]], vi = self$data[[self$options$samplingVariances]], mods = self$data[[self$options$moderatorcor]], slab = self$data[[self$options$slab]])
            data[[yi]] <- jmvcore::toNumeric(data[[yi]])
            data[[vi]] <- jmvcore::toNumeric(data[[vi]])
            data[[mods]] <- jmvcore::toNumeric(data[[mods]])
          } else {
            data <- data.frame(yi = self$data[[self$options$effectSize]], vi = self$data[[self$options$samplingVariances]], slab = self$data[[self$options$slab]])
            data[[yi]] <- jmvcore::toNumeric(data[[yi]])
            data[[vi]] <- jmvcore::toNumeric(data[[vi]])
          }
